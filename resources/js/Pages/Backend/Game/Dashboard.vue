                             ...
                             <div v-if="get(getTeamAns(task.userAnswer, team.teamCode), 'start_at')" class="flex gap-1">
                                <div>
                                    <template v-if="!get(getTeamAns(task.userAnswer, team.teamCode), 'value')">
                                        <input 
                                        :max="get(task, 'data.point')"
                                        type="number" 
                                        class="border flex-1 py-1 px-2 border-gray-600" 
                                        :placeholder="`Max point ${task.data.point}`" 
                                    /> 
                                        <button @click="(e) => {
                                            handleSubmit(e, game.id, task.id, team.teamCode, task.data.point)
                                        }" class="py-1 px-2 bg-green-400 border border-green-400">Save</button>
                                    </template>
