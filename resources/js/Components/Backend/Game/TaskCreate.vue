    ...
    <div class='p-5'>
        <div class="bg-white shadow rounded mt-5">
            ...
            <div class="p-4 flex gap-5">
                ...
                <button v-for="item in components" :key="item.name" @click="addTemplate(item, config)" >{{ item.name }}</button>
            </div>
        </div>
        ...
        <div class="flex bg-white shadow rounded mt-5">
            <div class="w-[350px] p-4 min-h-[90vh] max-h-[90vh] border-r flex flex-col gap-4 overflow-y-auto">
                <h1 
                    ...
                    @click="makeSelected(item, config)"
                >
                    <button class='absolute top-2 right-2' @click="removeItem(index, config)">
            ...
            <div class="w-full p-4">
                <component :is="getSelectedComponent(config)" />
            </div>
        </div>
    </div>
</template>
...
<script setup>
    ...
    const { components, makeSelected, addTemplate, removeItem } = useTaskCreate() 
    const { config } = useConnfiguration();

    const getSelectedComponent = (config) => {
        let selectedItem = config.find(item => item.isSelected); 
        if (selectedItem) {
